...
module Panel
  class PhotoUserReviewsController < ApplicationController
    ...
    def index
      # reviews for all photos in album and user is in album users
      photos = Photo.where(album_id: params[:album_id])
      reviews = PhotoUserReview.where(photo_id: photos.map(&:id))
      result = []
      reviews.each do |review|
        next unless AlbumUser.find_by(album_id: params[:album_id], user_id: review.user_id)
    ...
    def update
      ...
      if photo_user_review.nil?
        return if params[:review_id].nil?
